...
def test_customer_card_query_count_does_not_grow_with_documents(
    ...
    # Девять запросов: карточка сама по себе плюс постоянный счётчик новых
    # заявок клиентов в меню, плюс точечная проверка дублей по телефону
    # (duplicate_group_for_phone - одним запросом при отсутствии дублей) и
    # проверка права на слияние (is_manager). Важно не число, а то, что оно
    # одинаково при трёх и при восьми документах.
