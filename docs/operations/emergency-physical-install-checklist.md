...
Правило в репозитории закрывает это для новых копий, но файл, который уже лежит
на диске, оно не переписывает: Git считает копию актуальной, `git status`
показывает чистое дерево, а внутри WSL сценарий отвечает
`set: pipefail: invalid option name`.

Поэтому после получения выпуска нужен один явный шаг:
...
Сценарий сам находит нужные файлы, восстанавливает их из репозитория и
проверяет результат. Он ничего не сбрасывает и не чистит: рабочее состояние
станции и любые ваши правки остаются на месте. Повторный запуск безопасен.
...
шаг пропустить, установщик всё равно остановится перед запуском WSL и назовёт
эту же причину.
