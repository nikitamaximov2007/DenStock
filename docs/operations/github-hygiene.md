...
1. **`docker-compose.signing.yml`** больше не живёт только на сервере: он
   отслеживается в корне репозитория и побайтово совпадает с тем, что исполняет
   production. Секретом остаётся каталог с ключом, а не описание монтирования.
2. **Ночной бэкап** перенесён в `deploy/backup/`: скрипт
   `/usr/local/sbin/denstock-backup-capped` и оба unit-файла systemd сняты с
   работающего сервера побайтово. Раньше они не существовали ни в одной ветке,
   а именно они делают подписанные копии, на которых держится восстановление.
   Ветка `integration/canonical-backup-retention` источником не считается: там
   скрипт вчетверо короче, без блокировки, без `cleanup-hidden` и без учёта
   версий объектов в бакете.

Оставшиеся пробелы:

1. **Нет записи о том, что и когда выкладывалось.** Это закрывают теги выпусков.
2. **Ночные заметки о состоянии выпуска не отслеживаются Git.** Это осознанно,
   но значит, что новый ноутбук их не увидит: всё, что должно пережить машину,
   обязано попадать в `docs/`.

Секреты остаются снаружи: `.env` и `.env.backup` на сервере, конфигурация
rclone с ключами хранилища, приватный ключ подписи в
`/etc/denstock/manifest-signing`, а также сами резервные копии.
