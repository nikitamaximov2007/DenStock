...
Этот документ описывает подготовленный host-side слой. Production не настроен,
MAXINIK credentials не вводились, Codex login и Codex exec не запускались.
Действующий production guard в `apps.ai_support` не снят. До отдельного rebase,
интеграционного аудита и staging acceptance ИИ-поддержка в production должна
...
Codex binary должен быть отдельным regular file `/usr/local/bin/codex`,
`root:root`, без group/other write. Symlink и npm shim launcher отвергает. На
staging следует установить проверенный native artifact `0.142.5`, а не ослаблять
эту проверку.
...
## Installation: только staging после отдельного подтверждения
...
socket units уже active. Активный staging слой сначала останавливают и проверяют
отдельно; installer не делает скрытый restart.

`request-creator-uid` должен быть будущим non-root UID Django container на
host. Текущий production container нельзя подключать, пока он не переведён на
явный non-root UID/GID.
...
## Production checklist: только будущий отдельный этап

1. Получить подписанный staging acceptance по всем 20 пунктам.
2. Перебазировать ветку на финальный AI MVP и выполнить integration plan ниже.
3. Повторить independent security review, PostgreSQL gate и полный test suite.
4. Получить отдельное явное разрешение на production changes.
5. Сверить production UID/GID, regular Codex binary, mounts и отсутствие secrets
   в repository, Compose environment, logs и backups.
6. Сначала применить firewall, затем proxy, затем проверить fail-closed health.
7. Включать launcher socket только после exact `capabilities --json`.
8. Не снимать production guard до успешного handshake system check.
9. Выполнить smoke без private данных, проверить cleanup и остановку proxy.
10. Зафиксировать rollback window; при любой ошибке оставить AI support disabled.
...
`inet denstock_ai`, units, wrappers, private sing-box binary, installed launcher
code/docs и tmpfiles rule. Он намеренно сохраняет `/etc/denstock-ai`,
...
## Минимальный integration plan после rebase

Эта ветка не меняет Django provider, settings, checks или Compose. После rebase
на финальный `feature/ai-support-mvp` потребуется отдельный commit:

1. Запускать web container под фиксированным non-root UID.
2. Добавить только GID `denstock-ai-client`, без Docker/root privileges.
3. Bind-mount только launcher Unix socket и requests root; не монтировать
   `/etc/denstock-ai`, `CODEX_HOME`, proxy config или repository в launcher.
4. Добавить external adapter с protocol v1 и bounded framed JSON.
5. Adapter создаёт canonical UUID directory/schema/image, prompt передаёт по
   socket и не принимает path/model/environment от request.
6. System check запрашивает `capabilities --json`, проверяет exact schema,
   versions, `maxinik-proxy-only`, direct block и `ok`.
7. Только после Linux staging acceptance и отдельного review можно заменить
   безусловный production guard на handshake gate.
8. Повторить все AI tests, PostgreSQL concurrency tests и full quality gates.

Потенциальные конфликты ограничены будущим adapter/checks/Compose integration.
В этой ветке существующие `apps/ai_support/providers/codex_cli.py`, services,
checks и settings не изменяются.
