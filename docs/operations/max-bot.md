...
Результат приёмки 2026-09-17 (`MAX FORENSICS PASS`, один аккаунт MAX, заявки A и B):
ссылка B в существующем диалоге сразу привязала B и прислала её сводку;
`сообщение для B` попало только в B, `сообщение для A` только в A; два нажатия
одной кнопки выбора дали разные `callback_id` (116 символов); все `mid` по 36
символов и уникальны; B получила своё первое подтверждение, A повторного не
получила. Тип события при переходе в существующий диалог не записывается.
