...
## Качество ответов и контекст

Фактическая база знаний находится в `docs/ai-support/`. Она описывает реальные
пользовательские пути, подписи кнопок и полей, момент изменения остатков,
ограничения документов и роли. Старый app-internal набор коротких заметок не
используется.

`apps.ai_support.knowledge` детерминированно определяет тип вопроса и ранжирует
темы по keyword rules и allowlisted route name. Внешние embeddings, vector DB,
OpenAI API и платный API key не используются. В prompt попадают только
ограниченные тематические фрагменты, безопасный route context, роли владельца
разговора и bounded history этого же разговора.

Разрешённый контекст страницы: path без query, route name, пользовательское
название раздела и страницы, а для allowlisted detail route - числовой ID и тип
сущности. HTML, headers, cookies, CSRF, query-параметры, скрытые поля и данные
чужого разговора не передаются. При нехватке общего лимита старая история
сокращается, но текущий вопрос и system rules не обрезаются.

System prompt не задаёт единую форму ответа. Он требует естественный прямой
ответ, точные названия DenisStock, пошаговый формат для инструкций и один
уточняющий вопрос при неоднозначности. Read-only, no-tools и prompt-injection
