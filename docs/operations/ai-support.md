...
- запрет production-включения до появления внешнего unprivileged launcher;
...
## Будущий production launcher contract

Режим `external` в текущем commit всегда является configuration error. Абсолютный
путь к executable не доказывает наличие безопасного launcher. Будущий launcher
должен быть отдельным root-owned audited executable, а не shell-wrapper из этой
ветки. Контракт будущей реализации:
...
До реализации и отдельного аудита этого контракта system check не позволяет
включить `AI_SUPPORT_ENABLED` в production.
