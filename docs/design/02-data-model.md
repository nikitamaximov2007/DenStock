...
- **Зоны приёмки** (`is_receiving = true`) физически держат остаток на этапе
  приёмки, поэтому у них тоже `storage_allowed = true`. Флаг `is_receiving`
  лишь помечает их как буфер приёмки, не как обычное место хранения.
...
### UnresolvedScan (`core`) — нераспознанные сканы
`id`, `code`, `datetime`, `user` FK, `context` (экран, где сканировали),
`resolved` bool, `resolved_part` FK→PartType ✓null.
Питает виджет **«неопознанные детали»** на главной панели: коды, которые сканер
не смог сопоставить с деталью/экземпляром/партией/ячейкой. Закрывается созданием
карточки или привязкой к существующей.

> Виджет **«обнаруженные проблемы»** (ТЗ раздел 24) — **вычисляемый**, без
> отдельной таблицы: расхождения `check_stock_balance`, низкий остаток,
> истёкшие резервы, «зависшие» партии (приняты, но себестоимость не
> рассчитана), нераспознанные сканы.
...
2. **Изменение остатка только через `StockMovement`.** Прямое редактирование
   ...
   Поля `StockLot.quantity`, `PartItem.status`/`location`, весь `StockBalance` —
   **read-only в формах и в Django-админке**; менять их можно только сервисами
   движений. Нет ни одного экрана, где остаток правится «полем».
