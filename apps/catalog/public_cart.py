...
``signed_cookies`` and its database role cannot write. The cookie holds
nothing but ``{public_id: quantity}``; price, availability, article and name
...
for a part that has nothing available at submission time.
"""
...
def build_cart_view(session) -> CartView:
    ...
    for key, quantity in stored.items():
        ...
        if state == LINE_INQUIRY:
            ...
            counts["priced"] += 1
            total += line_total
        lines.append(CartLine(card=card, quantity=quantity, state=state, line_total=line_total))
    return CartView(
        lines=lines,
        total=total,
