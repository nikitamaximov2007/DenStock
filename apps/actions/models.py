...
class PartCustomsInfo(models.Model):
    ...
    class ApplicationArea(models.TextChoices):
        ...
        # Операторский список быстрых действий разделяет катер и лодочный
        # мотор: это разные таможенные категории для сотрудника. Прежнее
        # значение «КАТЕР / ЛОДКА» остаётся - его выдаёт автоопределение по
        # совместимости каталога, и удаление стёрло бы уже сохранённые данные.
        OUTBOARD_MOTOR = "ЛОДОЧНЫЙ МОТОР", "Лодочный мотор"
        BOAT_CRAFT = "КАТЕР", "Катер"
