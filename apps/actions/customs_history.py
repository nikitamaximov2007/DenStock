...
def line_chronological_key(line: dict) -> tuple:
    """Единый порядок операции: старые сверху, новые снизу.

    Один источник ключа для «Истории для таможенных заказов»
    (apps.customs_orders.services.customs_sources), таможенной
    Excel-выгрузки (apps.actions.services._customs_rows_from_lines) и
    запчастей на заказ (apps.ordered_parts.customs) - три места читали один
    и тот же смысл («дата операции, затем тип и id канонической строки»),
    но раньше каждое собирало свой кортеж заново. Тип операции и id строки -
    обязательный довесок: у продажи и ремонта нумерация своя, и без него две
    строки с одинаковой датой (или без даты) сравнивались бы произвольно.
    """
    return (line["occurred_at"] or _EPOCH, line["kind"], line["line_id"])
