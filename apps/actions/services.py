...
def _perform_action_atomic(
    *,
    ...
    if part.is_oil:
        # Сканер действий продаёт/выдаёт ОДНИМ шагом по part.recommended_price
        # как цене ЗА ЕДИНИЦУ - а для масла это цена ЗА УПАКОВКУ, и подстановка
        # её как цены за литр завысила бы сумму в разы. У масла нет здесь и
        # поля для ввода объёма (сканер молча берёт quantity=1). Явный отказ,
        # а не тихая порча цены - используйте карточку продажи/ремонта.
        raise ActionError(
            "Масло нельзя продать/выдать сканером действий: цена и объём "
            "для него другие. Откройте продажу или ремонт и добавьте масло "
            "там (раздел «Масло» - только объём, цена считается сама)."
        )
