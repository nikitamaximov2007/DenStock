...
Если доказать нечего, поле остаётся ПУСТЫМ, а сама операция из выгрузки не
исчезает: реальная продажа или ремонт обязаны попасть в Excel, и оператор
дозаполнит артикул вручную. Выдуманный номер в декларации хуже пустой клетки,
а потерянная строка хуже обоих.
"""
...
def article_snapshots(movements) -> dict[int, dict]:
    """Артикул на момент выбытия для каждого движения СКЛАДСКОГО ЖУРНАЛА.

    Диагностика сырого журнала, а не источник строк выгрузки: выгрузка идёт по
    строкам документов (см. ``customs_history``) и спрашивает про артикул
    ``article_numbers_by_document``. Здесь то же правило, только на ключе
    движения - им удобно разбирать конкретное движение в аудите.
    ...
    Если снимка нет - артикул НЕ ДОКАЗАН.
    """
...
def return_attributions(movements) -> dict[int, dict]:
    """Какое именно выбытие гасит каждый возврат в складском журнале.

    Тоже диагностика журнала. Выгрузка гасит возврат не движением, а строкой
    документа возврата (``source_sale_line``/``source_repair_line``) - ровно
    тем же правилом, что и отчёт «Продажи и ремонты».
    ...
    доказать нечем - они помечаются неоднозначными.
    """
    movements = list(movements)
    inbound = [
...
def article_numbers_by_document(sale_ids, repair_ids) -> dict[tuple, set[str]]:
    """Отсканированные номера по ключу «документ + деталь».

    Ключ документа, а не движения: строка продажи или выдачи в ремонт живёт в
    документе, и снимок сканера привязан к тому же документу. Ровно один номер
    в наборе - артикул доказан; ноль или несколько - доказать нечего.
    """
    from django.db.models import Q

    numbers: dict[tuple, set[str]] = {}
    rows = WarehouseAction.objects.exclude(part_number="").filter(
        Q(sale_id__in=list(sale_ids)) | Q(repair_order_id__in=list(repair_ids))
    ).values("sale_id", "repair_order_id", "part_type_id", "part_number")
    for row in rows:
        if row["sale_id"]:
            key = ("sale", row["sale_id"], row["part_type_id"])
            numbers.setdefault(key, set()).add(row["part_number"])
        if row["repair_order_id"]:
            key = ("repair", row["repair_order_id"], row["part_type_id"])
            numbers.setdefault(key, set()).add(row["part_number"])
    return numbers
