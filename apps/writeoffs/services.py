...
def available_quantity(part) -> Decimal:
    ...
    # The aggregate contains only lots that actually have an active reservation.
    # A normal unreserved lot is therefore absent from the mapping, not mapped
    # to Decimal("0").  Operator quick write-off must keep working for that
    # ordinary case instead of turning its availability rendering into a 500.
    bulk = sum(
