...
def _post_found_stock_group(*, entries, location, token: str, by=None):
    ...
    with transaction.atomic():
        ...
        for entry, quantity in prepared:
            ...
            if part.is_oil:
                # «+1 за скан» здесь всегда целое количество ЕДИНИЦ сканирования,
                # а для масла количество - литры: целое «3» было бы неоднозначно
                # (3 канистры? 3 литра?) и молча искажало бы остаток. Оприходуйте
                # масло обычной приёмкой партии или инвентаризацией ячейки.
                raise InventoryError(
                    f"Масло {exact_number} нельзя добавить пакетной приёмкой "
                    "«найдено» (целое количество здесь не может означать литры "
                    "однозначно). Используйте приёмку партии или инвентаризацию."
                )
            lot = (
                StockLot.objects.select_for_update()
