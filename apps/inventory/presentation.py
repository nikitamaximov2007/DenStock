...
def identity_for_part_ids(part_ids) -> dict[int, PartIdentity]:
    """Exact-артикул и производитель для набора id деталей (для отчётов/агрегатов).

    Константное число запросов на весь набор — для aggregate-строк
    (values()/annotate()), где самих объектов PartType в строках нет.
    """
    from apps.catalog.models import PartType

    parts = with_part_identity(
        PartType.objects.filter(pk__in=list(part_ids)), part_field=""
    )
    return {
        p.pk: PartIdentity(part_exact_number(p, default=""), manufacturer_display(p))
        for p in parts
    }
