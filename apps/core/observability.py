...
# кавычки съедаются целиком, а не обрываются на первом пробеле. Ключ тоже
# бывает в любых кавычках: repr словаря заголовков пишет 'Authorization': '...'.
# И с префиксом переменной окружения: MAX_BOT_TOKEN=..., MAX_WEBHOOK_SECRET=...
_SECRET_PATTERN = re.compile(
    r"(?i)([\"']?\b(?:[a-z0-9]+_)*(?:" + "|".join(_SECRET_KEYS) + r")\b[\"']?\s*[=:]\s*)"
    r"(\"[^\"]*\"|'[^']*'|[^\s,;&)}\]]+)"
)
