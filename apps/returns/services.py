...
def cancellation_allocations(lines, returned_by_line) -> list[ReturnAllocation]:
    ...
    Масло сюда НЕ попадает: его судьбу при отмене продажи задаёт отдельное
    явное решение оператора. Для ремонта масло считается использованным и
    обычным механизмом отмены не восстанавливается.
    """
    allocations = []
    for line in lines:
