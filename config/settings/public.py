...
authentication middleware. Its database role reads the catalog projections
and may write exactly one thing: a new customer request with its lines
(``apps.catalog.public_requests``). Migrations are run by the privileged
internal release job. The request stack, cookies and error handlers come from
